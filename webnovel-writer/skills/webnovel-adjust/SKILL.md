---
name: webnovel-adjust
description: 动态大纲调试与人工覆盖（极端修复）。当前主线未并入完整动态大纲链路，常规写作仍以主线真实能力为准。
---

# Dynamic Outline Adjustment（调试 / 极端修复模式）

## 定位声明

`/webnovel-adjust` 不是常规写作入口。

**状态说明（2026-03-30）**：动态大纲完整链路当前仍在 `feature/dynamic-outline` 分支，`main` 主线尚未并入完整解析/分析/执行模块。

**目标路径**：后续应由 `/zhizhang-write` 内嵌动态大纲能力。当前阶段只有在以下极端情况下才建议手动调用本 skill：

- 调试：大纲存在逻辑错误，需要人工定位根因
- 人工覆盖：系统自动处理无法满足的例外情况
- 极端修复：数据损坏、状态丢失等需要绕过正常流程的紧急处理

**正常写作循环**：`/zhizhang-write` → 审查反馈 → `/zhizhang-write` 下一章，不需要经过本 skill。

## Project Root Guard
- 必须先解析 `PROJECT_ROOT` 为真实书项目根（必须包含 `.webnovel/state.json`）

环境设置：
```bash
export WORKSPACE_ROOT="${CLAUDE_PROJECT_DIR:-$PWD}"

if [ -z "${CLAUDE_PLUGIN_ROOT}" ] || [ ! -d "${CLAUDE_PLUGIN_ROOT}/skills/webnovel-adjust" ]; then
  echo "ERROR: 未设置 CLAUDE_PLUGIN_ROOT" >&2
  exit 1
fi
export SKILL_ROOT="${CLAUDE_PLUGIN_ROOT}/skills/webnovel-adjust"
export SCRIPTS_DIR="${CLAUDE_PLUGIN_ROOT}/scripts"

export PROJECT_ROOT="$(python "${SCRIPTS_DIR}/webnovel.py" --project-root "${WORKSPACE_ROOT}" where)"
```

## Adjustment Types

### Type 1: 小修 - 修改某章情节
适用场景：某章的大纲内容需要修改（如筑基丹被偷 → 灵石被盗）

```bash
# 读取当前大纲
cat "$PROJECT_ROOT/大纲/第{volume_id}卷-详细大纲.md"
cat "$PROJECT_ROOT/大纲/第{volume_id}卷-时间线.md"
```

### Type 2: 中修 - 插入新章节/副本
适用场景：在指定位置插入新情节（如宗门大比10章副本）

```bash
# 插入后需要重新编号
# 所有后续章节的编号自动 +N
```

### Type 3: 大修 - 重写卷大纲
适用场景：整卷的核心冲突需要调整

## Workflow

### Step 1: 确认调整范围
```
用户：请调整第 {N} 章的大纲
原因：{用户描述的问题}

或：

用户：在第 {N} 章后插入 {M} 章的 {副本名} 副本
```

### Step 2: 分析影响
1. 读取当前大纲和时间线
2. 分析调整对后续章节的影响
3. 生成影响报告

**影响报告格式**：
```
调整范围：第 {N} 章
影响：
- 后续 {M} 章需要重新编号
- 时间线需要顺延 {X} 天
- 倒计时事件：宗门大比 D-{old} → D-{new}
```

### Step 3: 确认调整方案
展示调整后的：
- 章节大纲变化
- 时间线变化
- 需要更新的相关章节

### Step 4: 执行调整（使用 OutlineMutationEngine）
使用大纲写回引擎执行原子化调整：

```bash
# 调用 mutation engine 执行调纲
python "${SCRIPTS_DIR}/data_modules/outline_mutation_engine.py" \
    --project-root "${PROJECT_ROOT}" \
    --action-type "{minor_reorder|insert_arc|window_extend|manual_block}" \
    --trigger-chapter {N} \
    --reason "{调整原因}" \
    --impact-preview "{影响预览}"
```

**支持的 4 种动作类型**：
| 动作 | 说明 |
|------|------|
| `minor_reorder` | 小修：修改某章情节，不改变窗口 |
| `insert_arc` | 中修：插入新章节/副本，可能扩展窗口 |
| `window_extend` | 窗口扩展：扩大活动窗口范围 |
| `manual_block` | 阻断标记：标记需要人工审核的区块 |

**原子性保证**：
1. 先生成完整 `adjustment_record`
2. 先追加写入 `outline_adjustments.jsonl`（触发 window_version 增长）
3. 计算新的 `outline_runtime.json` 内容
4. 写入临时文件，原子替换
5. 最后更新 Markdown 大纲与时间线文件

**失败回滚**：
- `outline_runtime.json` 更新失败：不继续写 Markdown
- Markdown 写回失败：回滚 runtime，追加 rollback_reason="markdown_write_failed"

### Step 5: 验证
- 检查调整后的大纲一致性
- 检查时间线逻辑
- 检查倒计时是否正确
- 确认 `outline_runtime.json` 的 `window_version` 已增长

## 副本插入模板

当用户要求插入副本时，使用以下模板：

```markdown
## {副本名称}

**类型**: {战斗副本/情感副本/探秘副本}
**起始章节**: 第 {N} 章
**章节数**: {M} 章
**核心冲突**: {一句话描述}
**起始时间**: {时间锚点}
**结束时间**: {时间锚点}
**与主线关系**: {如何影响主线}

### 副本节拍
| 阶段 | 章节 | 内容 |
|------|------|------|
| 开局 | N~N+2 | 副本触发 |
| 发展 | N+3~N+6 | 困难推进 |
| 高潮 | N+7~N+8 | 最终对抗 |
| 收尾 | N+9~N+10 | 回归主线 |

### 副本内关键实体
- 新角色：
- 新物品：
- 新地点：
```

## 铺垫章节自动插入

当审查阶段检测到"关系跳跃"或"节奏太快"时，自动生成铺垫章节：

### 铺垫章节生成规则

| 问题类型 | 插入位置 | 章节数 | 内容要求 |
|---------|---------|-------|---------|
| 关系跳跃 | 当前章节前 | 3-5章 | 关系递进描写，至少3个关键事件 |
| 巧合过多 | 当前章节前 | 2-3章 | 补充道具/人物出场，解释"刚好" |
| 节奏太快 | 当前章节前 | 5-10章 | 拆分为小副本，起承转合完整 |

### 执行步骤

1. **读取当前大纲**：确定插入位置和影响范围
2. **生成铺垫章节**：按照模板生成章节大纲
3. **章节重新编号**：后续所有章节编号自动 +N
4. **更新时间线**：时间轴顺延
5. **更新节拍表**：插入新的节拍点
6. **验证一致性**：确保调整后无逻辑冲突

### 铺垫章节模板

```markdown
### 第 {N} 章：{标题}
- 目标: {20字以内}
- 阻力: {20字以内}
- 代价: {20字以内}
- 时间锚点: {相对时间，如"关系事件前3天"}
- 章内时间跨度: {如 "2小时"}
- 与上章时间差: {如 "紧接"}
- 爽点: {类型} - {30字以内}
- Strand: {Quest|Fire|Constellation}
- 反派层级: {无/小/中/大}
- 视角/主角: {主角A/女主/群像}
- 关键实体: {新增或重要出场}
- 本章变化: {30字以内}
- 章末未闭合问题: {30字以内}
- 钩子: {类型} - {30字以内}

**本章核心内容**：
{详细描述本章需要完成的关系铺垫/事件铺垫}
```

## 冲突检测规则

在调整时自动检测以下冲突：

| 冲突类型 | 检测逻辑 |
|---------|---------|
| 时间逆流 | 新时间 < 上一章时间 且 未标注闪回 |
| 物品凭空出现 | 物品在后续章节才首次出现但被使用 |
| 境界矛盾 | 境界变化不符合修炼逻辑 |
| 倒计时跳跃 | 倒计时非连续递减 |

## 大纲版本历史

每次调整后，自动记录到 `outline_adjustments.jsonl`：

```json
{
  "adjustment_id": "uuid",
  "trigger_chapter": 13,
  "adjustment_type": "minor_reorder",
  "reason": "修改第13章情节",
  "impact_preview": "修改大纲内容，不影响窗口",
  "before_window": {"start": 1, "end": 50, "version": 0},
  "after_window": {"start": 1, "end": 50, "version": 1},
  "written_at": "2026-03-29T10:00:00Z",
  "mainline_service_reason": null,
  "return_to_mainline_by": null
}
```

**版本增长规则**：
- `window_version` 的增长由 JSONL 成功追加 `adjustment_id` 触发
- 不由 runtime 文件写入触发
- `outline_runtime.json.last_applied_adjustment_id` 必须指向最后一条 `status=applied` 的记录

## 示例对话

**用户**: 第13章的大纲有问题，"筑基丹被偷"应该是"灵石被盗"

**Claude**:
```
明白，让我先读取第13章的当前大纲：
[读取大纲]

影响分析：
- 第13章内容修改
- 不影响后续章节编号
- 不影响时间线
- 确认是否同步修改 state.json 中的 item_states

调整确认：
- 第13章：灵石被盗（实验材料）代替筑基丹被偷
- 原因：练气三层无法持有筑基丹

执行调整？ (y/n)
```

**用户**: y

**Claude**:
```
✓ 已修改：第13章大纲
✓ 已更新时间线
✓ 已记录调整历史

后续章节无需调整。
```

## 内嵌行为说明

目标形态中，动态大纲能力应内嵌于 `/zhizhang-write` 主流程：

- Context Agent 在构建任务书时自动校验大纲约束（目标/冲突/承接/角色/场景约束/伏笔/追读力）
- 审查阶段（Step 3）检测到"关系跳跃"或"节奏太快"时，自动生成铺垫章节建议
- Data Agent 自动追踪大纲与实际正文的偏差，必要时在 state.json 中记录待修复项

在主线完整并入前，本 skill 仍应视为调试/人工兜底入口，而不是“已被主线自动替代”的能力说明。
