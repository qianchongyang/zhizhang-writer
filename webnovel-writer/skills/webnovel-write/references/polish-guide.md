...
### 第8层：Humanizer-zh 补充模式（中文AI词汇黑名单）

> 融合 Humanizer-zh 24模式分类，补充中文网文特有问题。

#### A. 中文AI高频词汇黑名单（网文特有问题）

以下词汇在 AI 网文中出现频率异常高，必须替换：

| 禁用词 | 替换方向 |
|--------|----------|
| 此外 | 删除或用"还有" |
| 至关重要 | 直接说"重要"或具体说明 |
| 深入探讨 | 改为具体研究动作"分析/验证/对比" |
| 强调 | 改为具体表达"说/指出/认为" |
| 持久的/增强 | 改为"长期/更强" |
| 培养 | 改为"练/学/积累" |
| 获得 | 改为"拿到/达到/取得" |
| 突出 | 改为"显眼/最强" |
| 相互作用 | 改为具体互动"互拼/互耗/配合" |
| 复杂/复杂性 | 改为具体描述"乱/难判断/多因素" |
| 格局 | 改为具体"局面/局势/情况" |
| 关键性的 | 改为"核心/重点" |
| 展示 | 改为"露出/表现/摆出来" |
| 织锦（抽象名词） | 改为具体"画面/内容" |
...
AI 为避免重复，会循环使用同义词，造成"绕着说"：

```
不合格（AI同义词循环）：
解决方案的核心在于深入分析问题的本质，
通过系统性的方法论，我们可以获得最佳的答案。
（"核心/本质/系统性/方法论/最佳"全是绕着说）

合格：
答案就在问题本身。多试几次就知道哪个管用了。
```
...
AI 常带"截至xxx年"或"根据我的训练数据"类痕迹：

```
不合格：
根据公开资料，以及截至2024年的研究显示……
据我所知，在过去的案例中……

合格：
资料上是这么写的。（不用声称知识来源的时间范围）
以前有过类似情况……
```
...
| 指标 | 通过线 | 致命线 | 备注 |
|-----|-------|--------|------|
| 因果连接词密度 | ≤2次/千字 | ≥5次/千字 | 统计”因此/然而/所以/由于/因而” |
| 短句(≤10字)占比 | 25-45% | <15% 或 >60% | 制造节奏停顿 |
| 停顿词密度 | 1-2次/500字 | <0.5次/500字 | 省略号/不完整句 |
| 三段式枚举句 | 0处 | ≥1处 | 首先/其次/最后 |
| 生理反应描写 | ≥1处/3段 | 全文<3处 | 手心出汗/喉咙发紧等 |
| 对白意图冲突率 | ≥60% | <30% | 有意图的对白占比 |
| 总结归纳词 | ≤1次/千字 | ≥3次/千字 | 由此可知/总之/不难发现 |

### 强制改写触发条件

满足以下任意一项，必须执行 AI痕迹替换算法：
- 致命线命中 ≥1项
- 通过线命中 ≥3项
- 外部AI检测预估分 >40%
...
- 量化阈值命中项必须在 deviation 中记录。

### Phase 1 终检（8层完整版）

- [ ] 高风险词汇已做全文检查与必要替换（1-7层）。
- [ ] 未出现三段式说明句（首先/其次/最后）。
- [ ] 对话存在真实冲突意图，不是信息宣讲。
- [ ] 第8层中文AI词汇黑名单已检查（至关重要/深入探讨/格局/证明等）。
- [ ] 同义词循环替换已检测（绕着说 → 直接说）。
- [ ] 破折号滥用已修复。
- [ ] 无”亲爱的读者/想必各位”等谄媚语气。
