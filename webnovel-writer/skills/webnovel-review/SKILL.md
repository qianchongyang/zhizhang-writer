...
推荐（bash）：
```bash
python3 "${SCRIPTS_DIR}/webnovel.py" --project-root "${PROJECT_ROOT}" workflow start-task --command webnovel-review --chapter {end} || true
```
...
```bash
python3 "${SCRIPTS_DIR}/webnovel.py" --project-root "${PROJECT_ROOT}" index save-review-metrics --data '@review_metrics.json'
```
...
```bash
python3 "${SCRIPTS_DIR}/webnovel.py" --project-root "${PROJECT_ROOT}" update-state -- --add-review "{start}-{end}" "审查报告/第{start}-{end}章审查报告.md"
```
...
在审查过程中，如果检测到以下情况，**必须建议触发 /webnovel-adjust**：

| 检测类型 | 触发条件 | 建议动作 |
|---------|---------|---------|
| 关系跳跃 | 角色关系突然变化（如刚认识就很亲密）且无铺垫 | 建议插入关系铺垫章节 |
| 巧合过多 | 关键道具/人物"刚好"出现，无合理来源 | 建议补充道具/人物出场铺垫 |
| 节奏太快 | 重要情节在1章内强行收尾，显得生硬 | 建议扩展为小副本（5-10章） |

**触发 /webnovel-adjust 的判断标准**：
- 情节自然度评分 < 6（10分制）
- 或 detected_issues 中包含 "关系跳跃" / "巧合过多" / "节奏太快"
...
建议：触发 /webnovel-adjust 调整大纲，在当前章节前插入 {N} 章关系铺垫

是否继续？
- 继续：触发 /webnovel-adjust，修改大纲后再继续
- 跳过：按当前大纲强行收尾（可能导致后续章节也生硬）
```
