...
**穿越文必检**：角色在公开/正式场合是否使用了原世界的术语。

**术语黑名单生成逻辑**：
1. 从 `state.json` 获取 `project_info.golden_finger_type` 或穿越设定
2. 从 `project_info.genre` 和 `project_info.core_selling_points` 判断世界背景
3. 生成对应的术语黑名单
...
| 世界背景 | 禁用术语 |
|---------|---------|
| 修仙世界 | 量子、物理、实验室、分析仪、频率、能量守恒、电磁场、DNA、基因、分子、化学反应 |
| 古代世界 | 手机、电脑、互联网、5G、WiFi、现代医学、原子弹、直播 |
| 末日世界 | 电网（末日前的）、互联网、直播、核电站、5G基站 |
| 异世界 | 以上均需根据设定调整 |

**危险信号** (WORLDVIEW_TERM_LEAK):
```
❌ "量子物理是你们那边的叫法？"
   → 违规角色: 主角
   → 语境: 公开场合（与掌门对话）
   → VIOLATION: 穿越者在公开场合暴露原世界术语

❌ 主角在宗门大比上大声宣布："我要用科学方法打败你们！"
   → 违规角色: 主角
   → 语境: 公开场合
   → VIOLATION: "科学"属于现代概念
```
...
**检测规则**：
1. 识别主角的"公开场合"：与长老/掌门对话、宗门集会、战斗演示等
2. 检测主角在公开场合的言论
3. 若包含黑名单术语，标记为 `high` 严重度

**严重度判定**：
| 场景 | 严重度 | 说明 |
|------|--------|------|
| 主角主动大声说出 | high | 主动暴露，不合理 |
| 被追问后回答 | medium | 被迫解释，可接受但仍有问题 |
| 内心独白 | low | 内心想法，无暴露风险 |
| 与信任的人私密对话 | medium | 有一定风险 |
...
| 章节 | 角色 | 违规术语 | 场景 | 严重度 |
|------|------|---------|------|--------|
| {M} | 主角 | "量子" | 公开场合（与掌门对话） | high |

**术语泄露分析**:
- 第{M}章主角在公开场合说出"量子"，暴露了原世界知识
- 建议：将"量子"替换为修仙世界能理解的解释（如"灵气波动"）
