...
- `README.md` 新增“重点能力”段，明确把动态大纲列为织章的核心能力之一
- 首页快速导航增加了动态大纲入口，便于用户第一眼看到这项能力
- 首页核心能力列表补充了“动态大纲：自动评估窗口变化、插入副本、阻断偏航”

**影响范围**

- 新用户和回访用户在首页就能看到动态大纲的定位，不再只在命令文档里埋着

**验证方式**

- 手动检查 `README.md` 中动态大纲的入口、摘要和锚点链接

### 2. 新增动态大纲手册

- 新增 [`docs/dynamic-outline-guide.md`](dynamic-outline-guide.md)，说明动态大纲的工作原理、日常使用方式、手动介入边界和常见问题
- `docs/README.md` 增加了动态大纲手册的阅读顺序和目录入口
- 动态大纲的默认窗口、`block_for_manual_review` 行为和日常写作流程都在手册中写清楚了

**影响范围**

- 使用者可以从首页直接进入动态大纲说明，不必翻命令文档找散落说明
- 对外发布时，动态大纲不再只是“内部实现细节”，而是有可读手册的正式能力

**验证方式**

- 手动检查 `docs/dynamic-outline-guide.md` 的内容结构和 `docs/README.md` 的链接顺序
